      ...
      <div class="schoolTable mt1">
        ...
        <el-dialog title="驾校信息维护" :visible.sync="dialogChangeForm">
          <el-form :model="changeForm">
            ...
            <el-form-item label="所在城市" :label-width="formLabelWidth">
              <el-select v-model="changeForm.region" placeholder="请选择活动区域">
                <el-option label="区域一" value="shanghai"></el-option>
                <el-option label="区域二" value="beijing"></el-option>
              </el-select>
              <el-select v-model="changeForm.region" placeholder="请选择活动区域">
        ...
        <el-table :data="tableData" style="width: 100%" :header-cell-style="getRowClass">
          ...
          <el-table-column prop="productNumber" label="套餐数量">
          </el-table-column>
          <el-table-column prop="customerNumber" label="学员数量">
          </el-table-column>
...
export default {
  ...
  methods: {
    // 查询
    getTableData() {
      this.$axios.get('/api/web/school/searchDriverSchool', this.formList).then(res => {
        console.log(res)
    ...
    //   this.$axios.post('web/kaola/changeRow', {id:index}).then(res => {
    ...
    getRowClass({ row, column, rowIndex, columnIndex }) {
      if (rowIndex == 0) {
        return 'background:#8895B4;color:#ffffff;'
      } else {
        return ''
      }
    }
